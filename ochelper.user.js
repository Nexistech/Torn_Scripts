...
let newbieExcludedCount = 0
let newbieExcludedList = []
let memberRosterCache = {}
let availableCrimeSlots = {}
let availableCrimeSlotsCount = 0
let alreadyCountedCrimeSlots = false
...
function showSettingsPage() {
  window.open("https://www.torn.com/preferences.php#OC2-Settings", "_blank")
}


async function loadMemberRosterCache() {
  try {
    if (isPDA()) {
      const raw = localStorage.getItem("CMR_OC2_memberRoster")
      memberRosterCache = raw ? JSON.parse(raw) : {}
      return
    }
    const raw = await GM.getValue("CMR_OC2_memberRoster", "{}")
    memberRosterCache = raw ? JSON.parse(raw) : {}
  } catch (err) {
    console.log("OC 2.0 Overview Script: failed to load member roster cache", err)
    memberRosterCache = {}
  }
}

async function saveMemberRosterCache() {
  try {
    const payload = JSON.stringify(memberRosterCache)
    if (isPDA()) {
      localStorage.setItem("CMR_OC2_memberRoster", payload)
    } else {
      await GM.setValue("CMR_OC2_memberRoster", payload)
    }
  } catch (err) {
    console.log("OC 2.0 Overview Script: failed to save member roster cache", err)
  }
}

function getNewbieExclusionDays() {
  const parsed = parseInt(userSettings?.newbieExclusionDays)
  return Number.isFinite(parsed) ? parsed : 30
}

function getRosterPruneDays() {
  const parsed = parseInt(userSettings?.rosterPruneDays)
  return Number.isFinite(parsed) ? parsed : 180
}

function estimateJoinTimestamp(daysInFaction, nowTs) {
  const days = parseInt(daysInFaction)
...
function getMemberDaysInFaction(memberId, apiDays, nowTs) {
  if (Number.isFinite(parseInt(apiDays))) {
    return parseInt(apiDays)
  }
  const cached = memberRosterCache[String(memberId)]
  if (cached && cached.joinTimestamp) {
    return Math.max(0, Math.floor((nowTs - cached.joinTimestamp) / 86400))
  }
  return null
}

function isNewbieExcludedFromAvailable(member) {
  const threshold = getNewbieExclusionDays()
...
function upsertMemberRosterEntry(memberId, name, apiDays, nowTs) {
  const key = String(memberId)
  const existing = memberRosterCache[key] || {}
  const apiJoinTs = estimateJoinTimestamp(apiDays, nowTs)
  let joinTimestamp = existing.joinTimestamp || apiJoinTs || nowTs
  // If API says they joined much more recently than we stored, they likely left and rejoined.
  if (apiJoinTs && existing.joinTimestamp && (apiJoinTs - existing.joinTimestamp) > 86400) {
    joinTimestamp = apiJoinTs
  } else if (apiJoinTs && !existing.joinTimestamp) {
    joinTimestamp = apiJoinTs
  }
  memberRosterCache[key] = {
    id: parseInt(memberId) || memberId,
    name: name || existing.name || "",
    joinTimestamp: joinTimestamp,
    lastSeen: nowTs,
    daysInFaction: Number.isFinite(parseInt(apiDays)) ? parseInt(apiDays) : (existing.daysInFaction ?? null)
  }
}

function pruneMemberRosterCache(currentMemberIds, nowTs) {
  const pruneAfter = getRosterPruneDays() * 86400
  if (!pruneAfter || pruneAfter <= 0) {
    return
  }
  const current = new Set((currentMemberIds || []).map(id => String(id)))
  for (const key of Object.keys(memberRosterCache)) {
    const entry = memberRosterCache[key]
    if (current.has(key)) {
      continue
    }
    const lastSeen = entry?.lastSeen || 0
    if ((nowTs - lastSeen) >= pruneAfter) {
      delete memberRosterCache[key]
    }
  }
}
...
function getLowSuccessThresholdForLevel(level) {
  const fallback = parseInt(userSettings?.warnLowSuccessPercentage)
  const safeFallback = Number.isFinite(fallback) ? fallback : 50
  const byLevel = userSettings?.warnLowSuccessByLevel
  if (!byLevel) {
    return safeFallback
  }
  const raw = byLevel[String(level)] ?? byLevel[level]
...
function checkMembersInCrimes(_data) {
  ...
  for (let i = 0; i < (_data.members).length; i++) {
    if (_data.members[i].status.state == "Fallen") {
      ...
      const _daysInFaction = getMemberDaysInFaction(_data.members[i].id, _data.members[i].days_in_faction, nowTs)
      seenMemberIds.push(_data.members[i].id)
      upsertMemberRosterEntry(_data.members[i].id, _data.members[i].name, _data.members[i].days_in_faction, nowTs)
  ...
  pruneMemberRosterCache(seenMemberIds, nowTs)
  saveMemberRosterCache()
  ...
  // Exclude recent joiners who are not currently in an OC
  for (const _key of Object.keys(memberInfo)) {
    const _member = memberInfo[_key]
    if (isNewbieExcludedFromAvailable(_member)) {
      if (!userSettings.memberIgnoreList.includes((_member.id).toString())) {
        skippedMemberCount += 1
        totalMembers -= 1
        availableMembers -= 1
      }
      newbieExcludedCount += 1
      newbieExcludedList.push({
        "id": _member.id,
        "name": _member.name,
        "daysInFaction": _member.daysInFaction
      })
      _member.excludeAsNewbie = true
    }
  }
}
...
function putAvailMembersIntoTable(_memberArray, _afterElm) {
  ...
  if (newbieExcludedList.length > 0) {
    if (_skipTitleText.length > 0) {
      _skipTitleText += "<br /><br />"
    }
    _skipTitleText += "Newbie exclusion period:"
    newbieExcludedList.forEach( (_newbie) => {
      const _days = (_newbie.daysInFaction == null) ? "?" : _newbie.daysInFaction
      _skipTitleText += (`<br />${_newbie.name} [${_newbie.id}] (${_days}d in faction)`)
    })
  }
  ...
  if (userSettings.memberIgnoreList.length > 0) {
    extraNotes.push(`${userSettings.memberIgnoreList.length} ignored`)
  }
  if (newbieExcludedCount > 0) {
    extraNotes.push(`${newbieExcludedCount} newbies`)
  }
  if (extraNotes.length > 0) {
    _ignoreText = (`(${extraNotes.join(", ")})`)
  }
    ...
    for (let i = 0; i < _crimeArray.length; i++) {
      ...
      for (let j = 0; j < (_crimeArray[i].slots).length; j++) {
        ...
        if (_crimeArray[i].slots[j].user) {
          ...
          const _lowSuccessThreshold = getLowSuccessThresholdForLevel(_crimeArray[i].difficulty)
          if (_crimeArray[i].slots[j].checkpoint_pass_rate < _lowSuccessThreshold) {
            _crimeSuccessWrapper = "OC2-lowSuccess"
          }
...
async function generateInsertHTML() {
  ...
  <div class="category-wrap OC2-memberViewer m-top10">
    <div class="title-black top-round t-overflow">OC 2.0 Overview <a href="https://www.torn.com/preferences.php#OC2-Settings" target="_blank"><span title="Go to Settings Page" class="extraSettingsButton">&#9881;</span></a><span class="hideLazyMembersButton OC2-hideAtStart"></span><span class="hideCrimesButton OC2-hideAtStart"></span></div>
  ...
  } else //this "else" is important to link the two if statements. Without it, it won't load on the faction -> crimes OC page because the second if statement takes precidence due to await
...
async function runOnceFunction() {
  ...
  if (userSettings.showAllenoneScript == "torn-probability-hide") {
    ...
    try {
      ...
      console.log("API Error", _err)
      return _err
    }
  }
  //prepare settings page if on correct URL
  if ($(location).attr("pathname").search("preferences") >= 0) {
    prepareSettingsPage()
    getSavedValues()
    settingsFillSelect()
...
async function testAPIKey(_APIKey) {
  ...
  console.log("OC 2.0 Overview Script: Sending API Request to test API Key on user request")
  let _insertHTML = ""
  return await $.ajax({
    dataType: "json",
...
async function setSavedValues() {
  let _userValues = {
    ...
    "warnLowSuccessPercentage": $(`input[id=warn-low-success-level-1]`).val() || 50,
    "warnLowSuccessByLevel": {
      "1": parseInt($(`input[id=warn-low-success-level-1]`).val()) || 50,
      "2": parseInt($(`input[id=warn-low-success-level-2]`).val()) || 50,
      "3": parseInt($(`input[id=warn-low-success-level-3]`).val()) || 50,
      "4": parseInt($(`input[id=warn-low-success-level-4]`).val()) || 50,
      "5": parseInt($(`input[id=warn-low-success-level-5]`).val()) || 50,
      "6": parseInt($(`input[id=warn-low-success-level-6]`).val()) || 50,
      "7": parseInt($(`input[id=warn-low-success-level-7]`).val()) || 50,
      "8": parseInt($(`input[id=warn-low-success-level-8]`).val()) || 50,
      "9": parseInt($(`input[id=warn-low-success-level-9]`).val()) || 50,
      "10": parseInt($(`input[id=warn-low-success-level-10]`).val()) || 50
    },
    ...
    "newbieExclusionDays": parseInt($(`input[id=newbie-exclusion-days]`).val()) || 0,
    "rosterPruneDays": parseInt($(`input[id=roster-prune-days]`).val()) || 180,
  }
...
function getSavedValues() {
  ...
  const _legacyWarn = userSettings?.warnLowSuccessPercentage
  const _byLevel = userSettings?.warnLowSuccessByLevel || {}
  for (let _lvl = 1; _lvl <= 10; _lvl++) {
    const _val = (_byLevel[String(_lvl)] ?? _byLevel[_lvl] ?? _legacyWarn ?? 50)
    $(`input[id=warn-low-success-level-${_lvl}]`).val(_val)
  }
  ...
  $(`input[id=newbie-exclusion-days]`).val(userSettings?.newbieExclusionDays ?? defaultUserSettings.newbieExclusionDays)
  $(`input[id=roster-prune-days]`).val(userSettings?.rosterPruneDays ?? defaultUserSettings.rosterPruneDays)
...
function prepareSettingsPage() {
  ...
  let _injectHTML = (`
    ...
    <div class="title-black top-round t-overflow OC2-settingsToggleVizButton">OC 2.0 Settings<div class="OC2-settingsToggleVizIndicator">&#9658;</div></div>
    <div class="cont-gray OC2-settingsTable OC2-hideAtStart"><ul class="table-body">
      ...
      <li class="table-cell OC2-settingsSection">
        <div class="OC2-settingsCell"><input id="OC2-APIInput" type="password" style="line-height: 14px; padding: 10px 8px" value="${_displayAPIKey}" /><div id="OC2-APITestButton" class="OC2-button">Test API Key</div></div>
      </li>
      <li class="table-cell OC2-settingsSection">
      ...
      <li class="table-cell OC2-settingsSection">
        <div class="OC2-settingsCell">
          ...
          <ul>
            ...
            <li><div class="OC2-settingsText"><div class="OC2-settingsLabel"><span title="Members who have been in the faction for fewer than this many days AND are not currently in an OC are hidden from the available-members list and counts.<br />Set to 0 to disable.<br />Join time comes from the existing faction members API (days_in_faction) and is cached locally by user ID so no extra API calls are made." class="OC2-infoHover">${moreInfoHover}</span>Newbie exclusion period:</div><input id="newbie-exclusion-days" type="number" min="0" style="line-height: 12px; padding: 5px" placeholder="30" size="10" /> days</div></li>
            <li><div class="OC2-settingsText"><div class="OC2-settingsLabel"><span title="Cached faction join dates are stored locally by user ID.<br />If a cached member has not been seen in the faction roster for this many days, they are removed from the cache." class="OC2-infoHover">${moreInfoHover}</span>Remove cached leavers after:</div><input id="roster-prune-days" type="number" min="1" style="line-height: 12px; padding: 5px" placeholder="180" size="10" /> days</div></li>
            <li><div class="OC2-settingsText"><div class="OC2-settingsLabel"><span title="A <span style='color:${colorObj.recolor.red[colorDisplayMode]}'>[%]</span> icon will appear next to a crime if a filled member slot is below the minimum success chance set for that crime's level (1-10)." class="OC2-infoHover">${moreInfoHover}</span>Member slot 'low success warning' threshold by crime level:</div></div>
              <div class="OC2-settingsText OC2-levelThresholdGrid" style="display:flex;flex-wrap:wrap;gap:6px 12px;margin:8px 0 8px 11px;max-width:720px;">
                <label style="display:inline-flex;align-items:center;gap:4px;">Lv1 <input id="warn-low-success-level-1" type="number" min="0" max="100" style="line-height: 12px; padding: 5px; width: 52px;" placeholder="50" /> %</label>
                <label style="display:inline-flex;align-items:center;gap:4px;">Lv2 <input id="warn-low-success-level-2" type="number" min="0" max="100" style="line-height: 12px; padding: 5px; width: 52px;" placeholder="50" /> %</label>
                <label style="display:inline-flex;align-items:center;gap:4px;">Lv3 <input id="warn-low-success-level-3" type="number" min="0" max="100" style="line-height: 12px; padding: 5px; width: 52px;" placeholder="50" /> %</label>
                <label style="display:inline-flex;align-items:center;gap:4px;">Lv4 <input id="warn-low-success-level-4" type="number" min="0" max="100" style="line-height: 12px; padding: 5px; width: 52px;" placeholder="50" /> %</label>
                <label style="display:inline-flex;align-items:center;gap:4px;">Lv5 <input id="warn-low-success-level-5" type="number" min="0" max="100" style="line-height: 12px; padding: 5px; width: 52px;" placeholder="50" /> %</label>
                <label style="display:inline-flex;align-items:center;gap:4px;">Lv6 <input id="warn-low-success-level-6" type="number" min="0" max="100" style="line-height: 12px; padding: 5px; width: 52px;" placeholder="50" /> %</label>
                <label style="display:inline-flex;align-items:center;gap:4px;">Lv7 <input id="warn-low-success-level-7" type="number" min="0" max="100" style="line-height: 12px; padding: 5px; width: 52px;" placeholder="50" /> %</label>
                <label style="display:inline-flex;align-items:center;gap:4px;">Lv8 <input id="warn-low-success-level-8" type="number" min="0" max="100" style="line-height: 12px; padding: 5px; width: 52px;" placeholder="50" /> %</label>
                <label style="display:inline-flex;align-items:center;gap:4px;">Lv9 <input id="warn-low-success-level-9" type="number" min="0" max="100" style="line-height: 12px; padding: 5px; width: 52px;" placeholder="50" /> %</label>
                <label style="display:inline-flex;align-items:center;gap:4px;">Lv10 <input id="warn-low-success-level-10" type="number" min="0" max="100" style="line-height: 12px; padding: 5px; width: 52px;" placeholder="50" /> %</label>
              </div>
            </li>
          </ul>
        </div>
  ...
  `)
  $("div.content-wrapper").append(_injectHTML)
  //onclick functions
  $(".OC2-hideAtStart").css({
    "display": "none"
  })
  $(".OC2-settingsToggleVizButton").off().on("click", event => {
      if ($(".OC2-settingsTable").is(":visible")) {
        $(".OC2-settingsTable").slideUp()
        $(".OC2-settingsToggleVizIndicator").html('&#9658;')
      } else {
        $(".OC2-settingsTable").slideDown()
        $(".OC2-settingsToggleVizIndicator").html('&#9660;')
      }
  })
  $("#OC2-APITestButton").off().on("click", (event) => {
...
function styleSettings() {
  ...
  $(".OC2-settingsToggleVizIndicator").css({
    "float": "right",
    "padding-right": "10px"
  })
  $(".OC2-settingsToggleVizButton").css({
    "cursor": "pointer",
  })
  $(".OC2-buttonDiv").css({
    "display": "flex",
